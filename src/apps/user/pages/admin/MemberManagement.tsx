...
import { Search, Filter, Edit2, Eye, UserPlus, Trash2, Plus } from 'lucide-react';
...
const MemberManagement = () => {
  ...
  useEffect(() => {
    const fetchMembers = async () => {
      try {
        const response = await userService.list(0, 50);
  ...
  const handleEdit = async (member: UserResponse) => {
    ...
    try {
      const updateData: Partial<UserResponse> = { email, fullName };
      if (passwordHash) {
        (updateData as any).passwordHash = passwordHash;
      }
      const response = await userService.update(member.id, updateData);
      setMembers((prev) => prev.map((item) => item.id === member.id ? { ...item, ...response } : item));
  ...
  const handleCreate = async () => {
    ...
    try {
      const response = await userService.create({ username, password, email, fullName, role: 'USER' });
      setMembers((prev) => [response, ...prev]);
      alert('Tạo người dùng mới thành công!');
    } catch (err: any) {
      console.error('Create failed:', err);
      alert(`Lỗi: Tạo người dùng thất bại`);
    }
  };
